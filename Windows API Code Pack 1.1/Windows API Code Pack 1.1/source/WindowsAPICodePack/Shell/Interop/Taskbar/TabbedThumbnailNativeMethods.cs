...
namespace Microsoft.WindowsAPICodePack.Taskbar
{
    internal static class TabbedThumbnailNativeMethods
    {
        ...
        internal static void SetIconicThumbnail(IntPtr hwnd, IntPtr hBitmap)
        {
        	...
        	//Boolean test = Bitmap.IsAlphaPixelFormat(bmp.PixelFormat);
		...
		520 // Return the opaque while pixel to prevent old foreground tab from being
	
		521 // shown when we cannot get the specified pixel.
	
		522 const int* tab_pixels = reinterpret_cast<int*>(tab_bitmap_.getPixels());
	
		523 if (!tab_pixels || x >= tab_bitmap_.width() || y >= tab_bitmap_.height())
	
		524 return 0xFFFFFFFF;
	
		525
	
		526 // DWM uses alpha values to distinguish opaque colors and transparent ones.
	
		527 // Set the alpha value of this source pixel to prevent the original window
	
		528 // from being shown through.
	
		529 return 0xFF000000 | tab_pixels[y * tab_bitmap_.width() + x];
...
/*
		private static IntPtr GetDrawedThumbnail()
{
	Bitmap bitmap(100, 100,  PixelFormat32bppARGB );
	...
	for (int i = 0; i < 100; ++i)
	{
		for (int j = 0; j < 100; ++j)
		{
			...
			{
				Color clrnew(255, i, j, i+j);
				bitmap.SetPixel(i, j, clrnew);
			}
		}
	}
